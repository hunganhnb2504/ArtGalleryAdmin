...
function ArtistDeleteAt() {
    ...
    //         try {
    //             const response = await api.get(url.ARTIST.TRASH);
    ...
    //xử lý khôi phục artist
    // const handleRestoreArtist = async (id) => {
    //     const userToken = localStorage.getItem("access_token");
    //     api.defaults.headers.common["Authorization"] = `Bearer ${userToken}`;
    //     try {
    //         const restoreResponse = await api.put(`${url.ARTIST.RESTORE.replace("{}", id)}`);
    //         if (restoreResponse.status === 200) {
    //             setArtists((prevArtists) => prevArtists.filter((artist) => artist.id !== id));
    //             toast.success("Restore Artist Successfully.", {
        ...
        <>
            {error ? (
                ...
                <>
                    ...
                    {loading ? <Loading /> : ""}
                    <Layout>
                        <Breadcrumb title="Artist Delete At" />
                                                    ...
                                                    {currentArtists.map((item, index) => {
                                                        return (
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <NavLink>
                                                                    {/* <NavLink onClick={() => handleRestoreArtist(item.id)} className="btn btn-success shadow btn-xs"> */}
