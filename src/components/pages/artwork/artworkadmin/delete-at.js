...
//             try {
//                 const response = await api.get(url.ARTWORK.TRASH);
...
//     //xử lý khôi phục artwork
//     const handleRestoreArtWork = async (id) => {
//         const userToken = localStorage.getItem("access_token");
//         api.defaults.headers.common["Authorization"] = `Bearer ${userToken}`;
//         try {
//             const restoreResponse = await api.put(`${url.ARTWORK.RESTORE.replace("{}", id)}`);
...
//                     <Layout>
//                         <Breadcrumb title="ArtWork Delete At" />
...
//                                                             <NavLink onClick={() => handleRestoreArtWork(item.id)} className="btn btn-success shadow btn-xs">
...
// export default ArtWorkDeleteAt;
